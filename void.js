...
function polygonPolygonCollision(a, b) {
    response.clear();
    return SAT.testPolygonPolygon(a.getSATPolygon(), b.getSATPolygon(), response);
}
function circlePolygonCollision(a, b) {
    // a is a circle, b is a polygon
    response.clear();
    // testPolygonCircle is slightly faster than testCirclePolygon, so we use it and reverse parameter order
    return SAT.testPolygonCircle(b.getSATPolygon(), a.getSATPolygon(), response);
}
...
var Player = (function() {
  ...
  function Player(x, y, radius, theta, speed, turnSpeed) {
    ...
    // Leech sucks up void energy and lowers each turn down to a minimum of zero. Players get leeched when they hit asteroids and when they consume black coins.
    this.leech = 0;
    this.leechRecovery = 0.01;
  ...
  Player.prototype.getSATPolygon = function() {
    var p1 = Vector2D.FromPolar(this.theta, this.circumRadius);
    var p2 = Vector2D.FromPolar(this.theta + 2/3*pi, this.circumRadius);
    var p3 = Vector2D.FromPolar(this.theta + 4/3*pi, this.circumRadius);
    var points = [p1, p2, p3].map(point => new SAT.Vector(point.x, point.y));
    return new SAT.Polygon(new SAT.Vector(this.pos.x, this.pos.y), points);
  };
  ...
  Player.prototype.updateVoid = function() {
    ...
    // Regenerate void.
    if (this.void < this.voidCap && this.leech === 0) {
    ...
    // Apply leech.
    if (this.leech > 0) {
        this.void -= this.leech;
        this.leech -= this.leechRecovery;
        if (this.leech < 0) {
            this.leech = 0;
        }
    }
    ...
    if (this.void < 0) {
        // Negative void means death!
        alert("You ran out of void energy: game over.");
        this.void = 0;
        this.pos = Vector2D.origin;
        this.void = 0;
        this.leech = 0;
    ...
    if (left && up && right && this.void > this.darkWhirlCost) {
      ...
      this.darkWhirling = false;
      if ((left || up || down || right) && this.void > this.voidFlameCost) {
  ...
  Player.prototype.displayVoid = function() {
    ...
    var voidPercentage = this.void / this.voidCap,
        leechPercentage = this.leech / this.voidCap;
    ...
    // Display the leeched void in dark purple.
    if (this.leech > 0) {
        fill(80, 20, 140);
        var rectifiedLeech = this.leech / this.leechRecovery, lostToLeechPercentage = rectifiedLeech * (rectifiedLeech - 1) / 2 * this.leechRecovery*this.leechRecovery;
        rect(1/10 * WIDTH*(2*voidPercentage - lostToLeechPercentage - 1), 0, 1/5 * WIDTH * lostToLeechPercentage, 1/25 * HALFHEIGHT);
    }
...
var DarkLaser = (function() {
  ...
  darkLaser.prototype.getPoints = function(pos) {
    if (pos === undefined) {
        pos = this.host.pos;
    }
  ...
  darkLaser.prototype.getSATPolygon = function() {
    points = this.getPoints(new Vector2D(0, 0)).map(pos => new SAT.Vector(pos.x, pos.y));
    return new SAT.Polygon(new SAT.Vector(this.host.pos.x, this.host.pos.y), points);
  };

  darkLaser.prototype.radialCheck = function(asteroid) {
      return (asteroid.pos.x - this.host.pos.x)**2 + (asteroid.pos.y - this.host.pos.y)**2 <= (12*this.host.radius + asteroid.radius)**2;
  };

  darkLaser.prototype.checkCollision = function(asteroid) {
    if (this.radialCheck(asteroid)) {
        return polygonPolygonCollision(this, asteroid);
    } else {
        return false;
    }
  };

  // Find whether the dark laser is intersecting with any asteroids.
  darkLaser.prototype.collide = function() {
      if (!this.activated) {
          return;
      }
      GC.asteroids.forEach(asteroid => {
          var collision = this.checkCollision(asteroid);
          if (collision) {
              asteroid.split(true);
          }
      });
...
var Asteroid = (function() {
  ...
  Asteroid.prototype.getSATPolygon = function() {
    var points = [];
    for (var i = 0; i < this.sides; i++) {
      var point = Vector2D.FromPolar(this.theta + 2/this.sides * pi * i, this.radius);
      points.push(new SAT.Vector(point.x, point.y));
    }
    return new SAT.Polygon(new SAT.Vector(this.pos.x, this.pos.y), points);
  };

  Asteroid.prototype.split = function(goodSplit) {
    if (goodSplit) {
        var radius = MAPRADIUS / 200;
        for (var i = 0; i < 4; i++) {
            GC.coins.push(new Coin(this.pos.x + (Math.random() - 0.5) * radius, this.pos.y + (Math.random() - 0.5) * radius, this.vel.getShifted(Vector2D.FromPolar(2*pi * Math.random(), this.radius/50*(Math.random() + 1))), radius));
        }
    } else {
        // Colliding badly means losing dark energy, which if it goes below zero is fatal.
        GC.player.leech = 1;
        // Rebound the player backwards.
        GC.player.vel.shift(GC.player.pos.getShifted(this.pos.getScaled(-1)).getNormalized().getScaled(this.radius / 8));
    }
    // First delete this asteroid. Then add new asteroids.
    for (var i = 0; i < GC.asteroids.length; i++) {
        if (GC.asteroids[i] === this) {
            GC.asteroids.splice(i, 1);
        }
    }
    if (this.radius > MAPRADIUS / 50) {
        var size = 1, change = 0;
        while (true) {
            change = Math.random() * 0.2 + 0.2;
            size -= change;
            if (size < 0) {
                break;
            }
            var newPos = this.pos.getShifted(Vector2D.FromPolar(2*pi * Math.random(), (Math.random() + 1)/2 * this.radius));
            GC.asteroids.push(new Asteroid(newPos, this.vel.getShifted(Vector2D.FromPolar(2*pi * Math.random(), WIDTH / 400)).getShifted(newPos.getShifted(this.pos.getScaled(-1)).getScaled(0.01)), this.radius * change, 2*pi * Math.random(), 1/60 * pi * (Math.random() - 1/2)));
        }
    }
  }
...
var Coin = (function() {
    ...
    function Coin(x, y, vel, radius) {
        this.pos = new Vector2D(x, y);
        this.vel = vel;
        this.radius = radius;
        this.theta = Math.random() * Math.PI*2;
        this.omega = (Math.random() - 0.5) / 5;
    }
    ...
    Coin.prototype.getSATPolygon = function() {
        return new SAT.Circle(new SAT.Vector(this.pos.x, this.pos.y), this.radius);
    };

    Coin.prototype.collide = function() {
        var collision = circlePolygonCollision(this, GC.player);
        if (collision) {
            // First, delete this coin.
            GC.coins.splice(GC.coins.indexOf(this), 1);
            // Next, reward the player with some learning!!
            //learn();
        }
    };

    Coin.prototype.attract = function() {
        // Coins get attracted to the given position when both far (inverse square) and close (inverse 2.8 power), so not quite like actual gravity.
        var differenceVector = this.pos.getShifted(GC.player.pos.getScaled(-1));
        var mag = differenceVector.getMagnitude();
        if (mag > MAPRADIUS / 10) {
            mag = MAPRADIUS / 10;
        } else if (mag < MAPRADIUS / 200) {
            mag = MAPRADIUS / 200;
        }
        this.vel.shift(differenceVector.getScaled(-10 / (mag**2)));
        if (!GC.player.darkWhirling) {
            // Repel from the player a little unless they are dark whirling.
            this.vel.shift(differenceVector.getScaled(200 / (Math.abs(mag - GC.player.radius)**2.8)));
        }
        this.vel.scale(0.99);
    }

    Coin.prototype.display = function(pos) {
        if (pos === undefined) {
            pos = this.pos.clone();
        }
...
var GameController = (function() {
  ...
  GameController.prototype.displayMinimap = function() {
    ...
    var objectScale = 1/18;
    ctx.scale(objectScale, objectScale);
    this.asteroids.forEach(asteroid => asteroid.display(asteroid.pos.getScaled((1/5 * HALFWIDTH/objectScale) / MAPRADIUS)));
    this.coins.forEach(coin => coin.display(coin.pos.getScaled((1/5 * HALFWIDTH/objectScale) / MAPRADIUS)));
    ctx.scale(1/objectScale, 1/objectScale);
    // The spaceship
    var shipScale = 1/6;
    ctx.scale(shipScale, shipScale);
    this.player.display(this.player.pos.getScaled((1/5 * HALFWIDTH/shipScale) / MAPRADIUS));
  ...
  GameController.prototype.updateBoundary = function() {
    ...
    this.asteroids.forEach(asteroid => this.beyondBoundary(asteroid));
    this.coins.forEach(coin => this.beyondBoundary(coin));
  };
